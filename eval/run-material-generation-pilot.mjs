...
// 실제 검색·생성은 앱의 lib 및 /api/generate를 직접 import하는 Vitest에 위임해
// 운영 파이프라인을 복제하지 않는다.
import { spawnSync } from "node:child_process";

const mode = process.argv[2] || "contract";
const allowedModes = new Set(["contract", "rag", "generation", "all"]);
if (!allowedModes.has(mode)) {
  console.error("사용법: node eval/run-material-generation-pilot.mjs [contract|rag|generation|all]");
  process.exit(2);
}
