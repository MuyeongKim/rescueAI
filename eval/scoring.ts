...
export const tutorEvalCaseSchema = z.object({
  ...
  expect: z.enum(["answer", "not_found", "refuse_medical"]).default("answer"),
  ...
  if (item.expect === "answer" && !item.keywordGroups?.length && !item.keywords?.length) {
...
export function scoreTutorEvalAnswer(
  ...
  if (item.expect === "not_found") {
    ...
    check("질문 반복을 제외한 설명 분량", body.replace(/\s/g, "").length >= item.minAnswerChars);
    ...
    for (const group of groups) {
      check(`답변 항목: ${group.label}`, group.anyOf.some((term) => body.includes(normalize(term))));
    }
