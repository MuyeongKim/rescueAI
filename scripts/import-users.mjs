...
//
// 사용법:
//   1) 엑셀 명단을 "CSV(UTF-8)"로 저장 — 열: email, full_name, division, role(선택: admin|user)
//      예) kim@jbfire.go.kr,김구조,전주 119구조대,user
//   2) node scripts/import-users.mjs <명단.csv>
//
// 동작: Supabase Auth에 계정 생성(이미 있으면 건너뜀) → profiles에 이름·소속·권한 반영.
// 비밀번호 없이 생성되며, 대원은 매직링크(이메일)로 로그인한다.
...
for (const [email, fullName, division, role] of rows) {
  ...
  const { error: profErr } = await supabase
    .from("profiles")
    .update({ full_name: fullName || null, division: division || null, role: safeRole })
    .eq("id", userId);
  if (profErr) {
